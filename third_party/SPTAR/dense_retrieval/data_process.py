...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare reduced-corpus files used by SPTAR / iGFT retrieval training."
    )
    parser.add_argument("--dataset", nargs="+", default=["msmarco", "fiqa"], help="BEIR dataset names.")
    parser.add_argument("--weak-num", nargs="+", default=["100k", "5000"], help="Weak query pool size tags.")
    parser.add_argument("--ratio", type=int, default=20, help="Positive-to-negative sampling ratio (default: 20).")
    ...
    for dataset_name in tqdm(args.dataset):
        ...
        for weak_num in args.weak_num:
            sample_corpus_v2(dataset_name, ratio=args.ratio, train_num=args.train_num, weak_num=weak_num)
